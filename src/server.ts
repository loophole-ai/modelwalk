...
function providersHandler(req: IncomingMessage, res: ServerResponse) {
...
function healthHandler(_req: IncomingMessage, res: ServerResponse) {
  res.writeHead(200, { 'Content-Type': 'text/plain' })
  res.end('OK')
}

function deprecatedHandler(_req: IncomingMessage, res: ServerResponse) {
...
const server = createServer((req, res) => {
  ...
  if (url === '/v2/providers') return providersHandler(req, res)
  if (url === '/providers') return deprecatedHandler(req, res)
  if (url === '/health' || url === '/healthz') return healthHandler(req, res)
